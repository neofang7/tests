...
#export PATH=$PATH:/opt/confidential-containers/bin
...
TEST_NAME="${TEST_NAME:-metrics-mlc-full}"
PAYLOAD="mlc.tar"
...
function k8s_run_a_mlc_case() {
    ...
    output=$(kubectl exec "${guest_image}" -- /bin/bash -c "${mlc_cmd}")
    sleep 10
...
function run_full_mlc() {
    guest_image=$(kubectl get po | grep "metrics-ubuntu" | awk '{print $1}')
    ...
    output=$(k8s_run_a_mlc_case "${guest_image}" "$cmd_idle_latency")
    ...
    output=$(k8s_run_a_mlc_case "${guest_image}" "$cmd_loaded_latency_R")
    ...
    output=$(k8s_run_a_mlc_case "${guest_image}" "$cmd_loaded_latency_W2")
    data=$(handle_mlc_output "${output}")
    ...
    output=$(k8s_run_a_mlc_case "${guest_image}" "$cmd_loaded_latency_W3")
    data=$(handle_mlc_output "${output}")
    ...
    output=$(k8s_run_a_mlc_case "${guest_image}" "$cmd_loaded_latency_W5")
    data=$(handle_mlc_output "${output}")
    ...
    output=$(k8s_run_a_mlc_case "${guest_image}" "$cmd_loaded_latency_W6")
    data=$(handle_mlc_output "${output}")
    ...
    output=$(k8s_run_a_mlc_case "${guest_image}" "$cmd_loaded_latency_W7")
    data=$(handle_mlc_output "${output}")
    ...
    output=$(k8s_run_a_mlc_case "${guest_image}" "$cmd_loaded_latency_W8")
    data=$(handle_mlc_output "${output}")
    ...
    output=$(k8s_run_a_mlc_case "${guest_image}" "$cmd_peak_bandwidth")
...
MLC_RESULT_DIR=${RESULT_DIR}/mlc
if [ ! -d ${RESULT_DIR} ]; then
    mkdir -p ${RESULT_DIR}
fi

if [ -d ${MLC_RESULT_DIR} ]; then
    rm -rf ${MLC_RESULT_DIR}
fi
mkdir -p ${MLC_RESULT_DIR}

for (( i=1; i <= ${round}; i++ ))
do
    run_full_mlc
    mv ${RESULT_DIR}/${TEST_NAME}.json ${MLC_RESULT_DIR}/${TEST_NAME}-${i}.json
done
