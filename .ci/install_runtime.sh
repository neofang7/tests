...
if [ "$TEE_CONFIDENTIAL_GUEST" == "false" ]; then
	echo "install_runtime configuration_legacy_metrics.toml"
       sudo cp metrics/configs/configuration_legacy_metrics.toml  /usr/share/defaults/kata-containers/configuration.toml
else
	echo "install_runtime configuration_tdx_metrics.toml"
       sudo cp metrics/configs/configuration_tdx_metrics.toml  /usr/share/defaults/kata-containers/configuration.toml
fi
